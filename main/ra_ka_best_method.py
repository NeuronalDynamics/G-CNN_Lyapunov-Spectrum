...
# OPT = μP/NTK-style per-layer LR: eta ~ 1 / (fan_in * depth^alpha).
...
# Optimizer scaling: eta = base_lr / (fan_in * depth^alpha)
BASE_LR             = 0.10
DEPTH_EXP_ALPHA     = 1.5
...
def ckpt_path(N: int, L: int, seed: int) -> str:
    return os.path.join(CHECKPOINT_DIR, f"model_N{N}_L{L}_seed{seed}.pt")
...
class FC(nn.Module):
    ...
    def forward(self, x, *, hid=False, grad=False):
        ...
        if hid:
            return x
        if grad:
            return self.out(x)   # logits (no tanh) for KA Jacobians
...
def per_layer_lr(layer: nn.Linear, base_lr: float) -> float:
    """μP/NTK-style: η ∝ 1 / fan_in (no depth factor)."""
...
# (optional) quick print to verify LRs
...
def verify_or_train_checkpoint(N: int, L: int, seed: int,
                               train_loader: DataLoader,
                               acc_target: float,
                               base_lr: float,
    ...
    if os.path.exists(path):
        ...
        if acc >= acc_target:
            ...
            print(f"[ckpt] loaded {os.path.basename(path)}  (acc={acc:.3f} < target) → continuing training")
...
def run_cell(N, L, seeds, train_loader, probe_loader):
    ...
    for s in seeds:
        ...
        net_init = FC(N, L).to(DEVICE)

        
        # when verifying/training checkpoints
...
if __name__ == "__main__":
    ...
    # Final summary only (no prescriptions)
