...
WIDTHS_F3 = np.logspace(0, 2.5, 14, dtype=int)   # 10 … 316
...
def make_circle_dataset(n_pts: int = TOTAL_PTS):
    ...
    #x_tr = (x_tr - μ) / σ
    #x_te = (x_te - μ) / σ
    # no normalisation in the original experiment
...
def ftle_field(model: FCNet, depth: int, grid: int = GRID_RES):
    ...
    xs = torch.linspace(-1.2, 1.2, grid, device=device)
    ys = torch.linspace(-1.2, 1.2, grid, device=device)
    ...
    with tqdm(total=total, desc="Fig 2 networks") as bar:
        for r, N in enumerate(WIDTHS_F2):
            for c, L in enumerate(DEPTHS_F2):
                ...
                np.save(f"ftle/ftle_w{N}_L{L}_seed0.npy", f)
                ax = axes[r, c]
...
def reproduce_figure3(data):
    ...
    for N in tqdm(WIDTHS_F3, desc="Fig 3b FTLE stats"):
        for L in DEPTHS_F3:
            ...
            r = np.sqrt(X**2 + Y**2)
            r_med = np.median(r)
            mask_d = np.isclose(r, r_med, atol=0.02)
            mask_c = r < 0.5*r_med
